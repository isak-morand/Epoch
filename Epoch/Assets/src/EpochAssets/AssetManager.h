...
namespace Epoch::Assets
{
	...
	public:
		...
		template<typename T>
		static std::shared_ptr<T> GetAssetAsync(TypedAssetHandle<T> aAssetHandle)
		{
			static_assert(std::is_base_of<Asset, T>::value, "GetAssetAsync only works for types derived from Asset");

			EPOCH_ASSERT(staticAssetManager, "AssetManager not set!");

			std::shared_ptr<Asset> asset = staticAssetManager->GetAssetAsync(aAssetHandle.Get());
			return std::dynamic_pointer_cast<T>(asset);
		}
