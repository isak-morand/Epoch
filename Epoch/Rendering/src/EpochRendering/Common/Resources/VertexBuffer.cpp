...
{
	VertexBuffer::VertexBuffer(const VertexBufferSpecification& aSpecification) : mySpecification(aSpecification)
	{
	...
	void VertexBuffer::SetData(Core::Buffer aBuffer, uint64_t aOffset)
	{
		if (!mySpecification.CpuWritable)
		{
			LOG_ERROR("SetData called on non-CPU-writable VertexBuffer '{}'.", mySpecification.DebugName);
			return;
		}
